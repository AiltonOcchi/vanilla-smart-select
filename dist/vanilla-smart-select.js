...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.VanillaSmartSelect = {}));
  ...
  const PT_BR = {
    ...
    // Selection limits
    maximumSelected: args => `Você só pode selecionar ${args.maximum} ${args.maximum === 1 ? "item" : "itens"}`,
    // AJAX (Phase 2)
    loading: "Carregando...",
  ...
  const ES = {
    ...
    searching: "Buscando...",
    searchPlaceholder: "Buscar...",
    inputTooShort: args => `Por favor, ingrese ${args.minimum} o más caracteres`,
  ...
  const DEFAULTS = {
    ...
    // Use null to allow i18n translation, or set a custom string to override
    matcher: null,
    // Custom matcher function
    matchStrategy: "contains",
    // 'startsWith', 'contains', 'exact'
    ...
    // Function to escape/sanitize HTML
  ...
  class Options {
    ...
    _validate() {
      ...
      if (typeof this._options.searchDelay !== "number" || this._options.searchDelay < 0) {
        throw new Error("searchDelay must be a non-negative number");
      }

      // Validate searchMinimumLength
      if (typeof this._options.searchMinimumLength !== "number" || this._options.searchMinimumLength < 0) {
        throw new Error("searchMinimumLength must be a non-negative number");
      }

      // Validate matcher
      if (this._options.matcher !== null && typeof this._options.matcher !== "function") {
      ...
      if (!validStrategies.includes(this._options.matchStrategy)) {
        throw new Error(`matchStrategy must be one of: ${validStrategies.join(", ")}`);
      }

      // Validate template functions
      if (this._options.templateResult !== null && typeof this._options.templateResult !== "function") {
        throw new Error("templateResult must be a function or null");
      }
      if (this._options.templateSelection !== null && typeof this._options.templateSelection !== "function") {
        throw new Error("templateSelection must be a function or null");
      }

      // Validate maximumSelectionLength
      if (typeof this._options.maximumSelectionLength !== "number" || this._options.maximumSelectionLength < 0) {
  ...
  class DataAdapter extends BaseAdapter {
    ...
    _normalizeOption(option) {
      ...
      const id = value !== "" && value != null ? value : value === "" ? "" : option.text;
      return {
        id: id,
        text: option.text,
    ...
    _isPlaceholder(item) {
      ...
      return item._isEmptyValue === true || item.id === undefined || item.id === null || item.id === "";
    }
    ...
    loadInitialSelection() {
      ...
      this.selection = Array.from(selectedOptions).map(opt => this._normalizeOption(opt));
    }
    ...
    _normalizeItem(item) {
      ...
      const normalized = {
        ...item,
        // Keep all custom properties (flag, code, emoji, avatar, etc.)
      ...
      if (item.children && Array.isArray(item.children)) {
        normalized.children = item.children.map(child => this._normalizeItem(child));
      }
    ...
    select(data) {
      ...
      if (!isMultiple) {
        ...
        if (!exists) {
          ...
          const maximumSelectionLength = this.options.get("maximumSelectionLength");
          if (maximumSelectionLength > 0 && this.selection.length >= maximumSelectionLength) {
            // Limit reached - emit event and return false
            const language = this.options.get("language");
            const message = typeof language.maximumSelected === "function" ? language.maximumSelected({
      ...
      this.trigger(this.$element, EVENTS.SELECT, {
        data
      });
      this.trigger(this.$element, EVENTS.CHANGE, {
        value: this.getValue()
      });
    ...
    unselect(data) {
      ...
      this.trigger(this.$element, EVENTS.UNSELECT, {
        data
      });
      this.trigger(this.$element, EVENTS.CHANGE, {
        value: this.getValue()
  ...
  class AjaxAdapter extends BaseAdapter {
    ...
    query(params = {}) {
      ...
      // Emit loading event
      this.emit(EVENTS.AJAX_LOADING, {
        params
      });
      this.trigger(this.$element, EVENTS.AJAX_LOADING, {
        params
      });

      // Prepare request parameters using custom data function
      const requestParams = this.ajaxConfig.data({
        term,
        page
      });

      // Use custom transport if provided, otherwise use default fetch
      const request = this.ajaxConfig.transport ? this.ajaxConfig.transport.call(this, requestParams, this.ajaxConfig) : this._defaultTransport(requestParams);
      this.currentRequest = request;
      return request.then(response => {
        this.currentRequest = null;

        // Process results using custom function
        const processedResults = this.ajaxConfig.processResults(response, params);

        // Normalize the results
        const normalizedResults = {
          results: Array.isArray(processedResults.results) ? processedResults.results.map(item => this._normalizeItem(item)) : [],
        ...
        if (error.name !== "AbortError") {
          this.emit(EVENTS.AJAX_ERROR, {
            error,
            params
          });
          this.trigger(this.$element, EVENTS.AJAX_ERROR, {
            error,
            params
          });
        }
  ...
  function applyTemplate(templateFn, item, targetElement, options = {}) {
    ...
    try {
      ...
      if (typeof customContent === "string") {
        ...
        const safeContent = typeof escapeMarkup === "function" ? escapeMarkup(customContent) : customContent;
        targetElement.innerHTML = safeContent;
        return true;
      }

      // Invalid return type - use fallback
      console.warn("Template function returned invalid content type:", typeof customContent);
      _renderDefault(targetElement, defaultText, useTextContent);
      return false;
    } catch (error) {
  ...
  class Selection {
    ...
    renderPlaceholder() {
      ...
      if (placeholder) {
        const placeholderEl = createElement("span", {
    ...
    _renderSingle(item) {
      ...
      applyTemplate(this._cachedTemplateSelection, item, rendered, {
        defaultText: item.text,
        useTextContent: true,
        // Use textContent directly for better performance
      ...
      if (allowClear) {
        ...
        const clearBtn = createElement("span", {
    ...
    _renderChoice(item) {
      ...
      const language = this.options.get("language");
      const removeLabel = typeof language.removeItem === "function" ? language.removeItem({
        text: item.text
      }) : `Remove ${item.text}`;
  ...
  class SelectionAdapter extends BaseAdapter {
    ...
    _bindEvents() {
      ...
      this._toggleClickHandler = e => {
        ...
        if (e.target.classList.contains("vs-selection__choice__remove") || e.target.classList.contains("vs-selection__clear")) {
          return;
        }
        if (this.instance) {
      ...
      this._removeClickHandler = e => {
        if (e.target.classList.contains("vs-selection__choice__remove")) {
          ...
          const item = this.dataAdapter.current().find(item => String(item.id) === String(id));
          if (item) {
    ...
    destroy() {
      ...
      if (this.selectionContainer) {
        if (this._toggleClickHandler) {
          this.selectionContainer.removeEventListener("click", this._toggleClickHandler);
          this._toggleClickHandler = null;
        }
        if (this._clearClickHandler) {
          this.selectionContainer.removeEventListener("click", this._clearClickHandler);
          this._clearClickHandler = null;
        }
        if (this._removeClickHandler) {
          this.selectionContainer.removeEventListener("click", this._removeClickHandler);
          this._removeClickHandler = null;
        }
        this.selectionContainer = null;
      }
  ...
  class SearchBox {
    ...
    _getPlaceholder() {
      ...
      return customPlaceholder !== null && customPlaceholder !== undefined ? customPlaceholder : language.searchPlaceholder || "Search...";
    }
  ...
  class DropdownAdapter extends BaseAdapter {
    ...
    bind(container, anchor) {
      ...
      if (searchable) {
        ...
        dropdownElement.insertBefore(searchBoxElement, dropdownElement.firstChild);
      }
    ...
    _bindEvents() {
      ...
      if (this.searchBox) {
        ...
        const searchDelay = ajaxConfig ? ajaxConfig.delay || 250 : this.options.get("searchDelay") || 0;

        // Create query handler (with debounce for AJAX)
        const handleQuery = term => {
          this.instance.emit(EVENTS.QUERY, {
            term
          });
        };

        // Use debounce for AJAX queries, immediate for local filtering
        const queryHandler = searchDelay > 0 ? debounce$1(handleQuery, searchDelay) : handleQuery;
    ...
    _handleOutsideClick(e) {
      ...
      const isClickInside = this.container.contains(e.target) || dropdownEl.contains(e.target);
      if (!isClickInside) {
        this.instance.close();
      }
    }
  ...
  class ResultsList {
    ...
    _renderItem(item, index, isGroupChild = false) {
      const className = isGroupChild ? "vs-result vs-result--group-child" : "vs-result";
      ...
      applyTemplate(this._cachedTemplateResult, item, result, {
        defaultText: item.text,
        useTextContent: false,
        // Use createElement wrapper for consistency
        escapeMarkup: this._cachedEscapeMarkup
      });
      return result;
    }
     ...
     */
    _renderNoResults() {
      const message = this.options.get("language.noResults") || "No results found";
    ...
    highlight(index) {
      ...
      const previousHighlight = this.container.querySelector(".vs-result--highlighted");
    ...
    getHighlighted() {
      ...
      if (this.flatResults && this.highlightedIndex >= 0 && this.highlightedIndex < this.flatResults.length) {
  ...
  class SearchManager {
    ...
    search(items, term) {
      ...
      items.forEach(item => {
        ...
        if (item.children && Array.isArray(item.children)) {
          const filteredChildren = item.children.filter(child => this.matcher(term, child));
  ...
  class ResultsAdapter extends BaseAdapter {
    ...
    _bindEvents() {
      ...
      this._clickHandler = e => {
        ...
        const allResults = this.dataAdapter.query({
          term: ""
        });
    ...
    _updateWithLocalData(term) {
      ...
      // Get all data
      const allData = this.dataAdapter.query({
        term: ""
      });
      ...
      if (tagsEnabled && term && term.trim()) {
        ...
        if (tag) {
          ...
          const tagExists = filteredResults.some(item => String(item.id).toLowerCase() === String(tag.id).toLowerCase());
          if (!tagExists) {
    ...
    _updateWithAjax(term, append = false) {
      ...
      return this.ajaxAdapter.query({
        ...
        // Auto-highlight selected item if exists, otherwise first non-disabled item
        if (this.accumulatedResults && this.accumulatedResults.length > 0 && !append) {
    ...
    showLoadingMore() {
      ...
      this.hideLoadingMore();
      const language = this.options.get("language");
      const loadingMoreMessage = language.loadingMore || "Loading more results...";
    ...
    hideLoadingMore() {
      const resultsContainer = this.results.getContainer();
      if (!resultsContainer) return;
      const loadingMoreEl = resultsContainer.querySelector(".vs-results__loading-more");
      if (loadingMoreEl) {
    ...
    _showLimitMessage(message) {
      ...
      const existingMessage = resultsContainer.querySelector(".vs-results__limit-message");
      if (existingMessage) {
    ...
    _showInputTooShortMessage(minimum) {
      ...
      if (!resultsContainer) return;
      const language = this.options.get("language");
      const message = typeof language.inputTooShort === "function" ? language.inputTooShort({
        minimum
    ...
    showError(error) {
      ...
      const language = this.options.get("language");
      const errorMessage = language.errorLoading || "The results could not be loaded";
    ...
    destroy() {
      ...
      if (this.resultsContainer) {
        ...
        if (this._mouseoverHandler) {
          this.resultsContainer.removeEventListener("mouseover", this._mouseoverHandler);
          this._mouseoverHandler = null;
        }
        if (this._scrollHandler) {
          this.resultsContainer.removeEventListener("scroll", this._scrollHandler);
          this._scrollHandler = null;
        }
        this.resultsContainer = null;
      }
  ...
  class KeyboardManager {
    ...
    _bindSelectionEvents() {
      ...
      this.selectionElement.addEventListener("keydown", this._selectionKeydownHandler);
    }
    ...
    destroy() {
      ...
      if (this.selectionElement && this._selectionKeydownHandler) {
        this.selectionElement.removeEventListener("keydown", this._selectionKeydownHandler);
        this._selectionKeydownHandler = null;
      }
  ...
  class AccessibilityManager {
     ...
     */
    _setupARIA() {
      if (!this.selectionElement || !this.dropdownElement || !this.resultsElement) return;
      ...
      const language = this.options.get("language");
      this.dropdownElement.setAttribute("aria-label", language.selectOptions || "Select options");
    ...
    announceResultsCount(count) {
      ...
      if (count === 0) {
        ...
        const message = typeof language.resultsAvailable?.other === "function" ? language.resultsAvailable.other({
          count
    ...
    announceSelection(item) {
      if (!item) return;
      const language = this.options.get("language");
      const message = typeof language.selected === "function" ? language.selected({
        text: item.text
      }) : `Selected: ${item.text}`;
      this.announce(message);
    }
    ...
    updateActiveDescendant(index) {
      ...
      if (index >= 0 && index < results.length) {
        ...
        this.selectionElement.setAttribute("aria-activedescendant", activeItem.id);
      } else {
  ...
  class VanillaSmartSelect extends EventEmitter {
    ...
    _init() {
      ...
      if (this.element.tagName !== "SELECT") {
        throw new Error("VanillaSmartSelect must be initialized on a <select> element");
      }
      ...
      this.trigger(this.element, EVENTS.INIT, {
        instance: this
      });
    ...
    _initializeAdapters() {
      ...
      this.resultsAdapter.bind(dropdownContainer, this.dataAdapter, this.dropdownAdapter, this.ajaxAdapter);
    }
    ...
    _initializeManagers() {
      ...
      this.keyboardManager.bind(selectionElement, searchInput, this.resultsAdapter);
      ...
      this.accessibilityManager.bind(selectionElement, dropdownElement, resultsElement);
    }
    ...
    open() {
      ...
      const openingAllowed = this.trigger(this.element, EVENTS.OPENING, {}, {
        cancelable: true
      });
      if (!openingAllowed) {
        return;
    ...
    close(options = {}) {
      ...
      const closingAllowed = this.trigger(this.element, EVENTS.CLOSING, {}, {
        cancelable: true
      });
      if (!closingAllowed) {
        return;
    ...
    clear() {
      ...
      const clearingAllowed = this.trigger(this.element, EVENTS.CLEARING, {}, {
        cancelable: true
      });
      if (!clearingAllowed) {
        return this;
    ...
    addOption(option) {
      ...
      // Get current data
      const currentData = this.dataAdapter.query({
        term: ""
      });
    ...
    removeOption(id) {
      ...
      // Get current data
      let currentData = this.dataAdapter.query({
        term: ""
      });
    ...
    updateLanguage(language) {
      if (!language || typeof language !== "object") {
        console.warn('updateLanguage expects a language object. Use getLanguage("code") to get one.');
        return this;
      }
      ...
      if (this.dropdownAdapter && typeof this.dropdownAdapter.update === "function") {
        this.dropdownAdapter.update();
      }
      if (this.selectionAdapter && typeof this.selectionAdapter.update === "function") {
        this.selectionAdapter.update();
      }
      return this;
    }
...
//# sourceMappingURL=vanilla-smart-select.js.map
