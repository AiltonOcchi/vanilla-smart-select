...
- **Versão hardcoded em ~30+ arquivos da doc** (Lote release 1.0.5): `docs/en/*.html`, `docs/pt/*.html`, `README.md`, `README.pt-BR.md` e `src/index.js` carregam a string da versão (`v1.0.4` → `v1.0.5`) inline em sidebars, badges, CDN URLs e JSDoc. A cada release alguém precisa fazer substituição em massa. Considerar centralizar (ex.: `docs/js/version.js` lido por um pequeno script de injeção, ou um pre-build que regenera as páginas a partir de templates). Dívida operacional, não breaking — não precisa esperar v2.0.
- **Opções cacheadas vs. `Options.set()` em runtime** (Lote 3): `templateSelection`, `templateResult` e `escapeMarkup` são lidos uma vez no construtor de `Selection`/`ResultsList` e cacheados em propriedades `_cachedXxx`. O padrão é coerente entre as três opções. Mas `Options.set(key, value)` ([core/Options.js:152](src/core/Options.js#L152)) é uma superfície pública (mesmo não documentada) que funciona para `disabled`/`multiple`/`language` (têm fluxos de refresh — `enable`/`disable`/`updateLanguage`) e é **silenciosamente ineficaz** para opções cacheadas: o usuário chama `instance.options.set('templateSelection', novaFn)`, não recebe erro, mas renderizações futuras continuam usando a função antiga. Não é bug — é design (runtime change não é suportado). Mas é trap latente. Candidato a documentar explicitamente em v2.0, ou a expor um método `update()` genérico que invalide caches.
